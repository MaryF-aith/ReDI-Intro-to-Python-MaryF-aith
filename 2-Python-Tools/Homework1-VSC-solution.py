#This is my first code
print("Hello Intro to Computer Science")
#Task 1: Write a print statement which prints "Hello world!"
print("Hello world!")
#Task 2: Assing your name to a variable which is called myname, then print the variable
myname="nesli"
print(myname)
#Task 3: Write a print statement which prints "Hello yourname" by combining string and variable
print("Hello",myname)
#Task 4: Write a print statement which prints "Hello yourname" by combining string and variable, f-string
print(f"Hello {myname}")